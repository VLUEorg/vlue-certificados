...
def make_gradient_strip(width: int, height: int = 48) -> Image.Image:
    """Recrea el separador original: línea azul fina con diamante en el centro."""
    img  = Image.new('RGBA', (width, height), (0, 0, 0, 0))
    draw = ImageDraw.Draw(img)

    cx  = width  // 2
    cy  = height // 2
    R, G, B  = 91, 183, 221   # azul del separador original
    diamond  = 10             # mitad del ancho/alto del diamante
    fade_end = cx - diamond - 20

    # Líneas horizontales a izquierda y derecha del diamante
    for x in range(width):
        if cx - diamond <= x <= cx + diamond:
            continue   # hueco donde va el diamante
        # distancia al borde más cercano del diamante
        dist_center = min(abs(x - (cx - diamond)), abs(x - (cx + diamond)))
        dist_edge   = min(x, width - x)
        alpha = int(210 * min(1.0, dist_edge / max(fade_end, 1)))
        for y in [cy - 1, cy, cy + 1]:
            draw.point((x, y), fill=(R, G, B, alpha))

    # Diamante central (rombo relleno)
    pts = [
        (cx,          cy - diamond),   # arriba
        (cx + diamond, cy),            # derecha
        (cx,          cy + diamond),   # abajo
        (cx - diamond, cy),            # izquierda
    ]
    draw.polygon(pts, fill=(R, G, B, 200))
    # Brillo interior del diamante
    pts_inner = [
        (cx,              cy - diamond + 4),
        (cx + diamond - 4, cy),
        (cx,              cy + diamond - 4),
    ...
    draw.polygon(pts_inner, fill=(200, 235, 250, 160))
...
def gradient_separator(img: Image.Image, cx: int, y: int, xl: int, xr: int):
    ...
    if os.path.exists(GRADIENT):
        strip = Image.open(GRADIENT).convert('RGBA').resize(
            (target_w, target_h), Image.Resampling.LANCZOS)
    else:
        strip = make_gradient_strip(target_w, target_h)
    img.alpha_composite(strip, (xl, y - target_h // 2))
